...
struct LightStruct  //struct with lightinfo, easier to work with.灯光信息
{
	float4 LightVec;
...
float4x4 WorldViewProj;		//MVP矩阵			
LightStruct lightsarray[3]; //3-light array灯光数组
...
bool bAnisotropicSpec
<
    string gui = "slider";
    string UIName = "     Anisotropic";
> = false;
...
<
    string gui = "slider";
    string UIName = "Tangent Map";
> = false;


texture tangentMap : TANGENTMAP
<
	string name ="";
	string UIName = "     Tangent Map";
	string ResourceType = "2D";
...
int TangentMapUV
<
string UIName = "         Tangentmap UV Channel";
string UIType = "FloatSpinner";
float UIMin = 1;
float UIMax = 4;
float UIStep = 1;
...
bool bFlipTangentGreenChannel
<
    string gui = "slider";
    string UIName = "     Flip Green";
> = true;

bool bFlipTangentRedChannel
<
    string gui = "slider";
    string UIName = "     Flip Red";
> = true;

bool bRoughTangents
<
    string gui = "slider";
    string UIName = "     Rough Tangents";
> = true;
...
//standard diffuse lighting by dot product 漫反射函数
...
//Half Lambert lighting, function by Valve Software.半阿尔伯特函数
...
float4 anisotropicspecular(float3 normal, float3 tangent, float3 lightvec, float3 eyevec, float4 glossiness)
{
	...
	float4 specular;
	//半向量与切线向量夹角的正弦
	specular = sqrt( 1 - pow(dot(halfvector, tangent),2) ); // root(1 - (half dot tangent)^2) is anisotropic specular正弦
...
float3 TangentApplication(float3 tangent, VS_To_PS_Struct In)
{
	float3 fixedtangent = tangent;
	
    const float3x3 basis = float3x3( normalize( In.worldTangent ), normalize( In.worldBinormal ), normalize( In.worldNormal ) );
    fixedtangent = normalize( mul( tangent, basis ) ); //big thanks to CW for showing me the working code for this.
	
	return	normalize( fixedtangent ) ;
}
...
{ 
	...
	if(bUseNormalMap)
	{
		...
		normal = normal.yxz;
		if (bFlipGreenChannel) normal.r =-normal.r; //flip green for texture instead of flipping tangent, works for object space normals also
		/*
		if (bFlipRG) 
		{
			float temp = normal.r;
	...
	half2 tangentUV = In.texCoord.xy;
	if(TangentMapUV == 2)	tangentUV=In.texCoord.zw;
	else if(TangentMapUV == 3) tangentUV=In.texCoord2.xy;
	else if(TangentMapUV == 4) tangentUV=In.texCoord2.zw;

	float3 tangent = In.worldNormal; //default worldspace vertex normal
	if(bUseTangentMap)
	{
		tangent = tex2D(tangentSampler, tangentUV).rgb;
		tangent = (2*tangent)-1; //expand into -1 to 1 range, since a normalmap is always returned in 0 to 1 space
		tangent = tangent.yxz;
		if (bFlipTangentGreenChannel) tangent.g = -tangent.g;
		if (bFlipTangentRedChannel) tangent.r = -tangent.r;
		
		if (bRoughTangents) tangent = normalize(tangent + tex2D(specularSampler, diffUV) * 0.15f);
	}
	tangent = TangentApplication(tangent, In);
	...
	for(int i = 0; i < numberOfActiveLights; ++i) //for loop to iterate over our 3 lights
	{
		...
		if (bAnisotropicSpec)
			specular *= anisotropicspecular(normal, tangent,lightsarray[i].LightVec, In.eyeVec, gloss);
		else
...
{ 
	...
	{
		gloss = 1-desaturate(tex2D(glossinessSampler, diffUV)); //overwrite glossiness global with texsample
